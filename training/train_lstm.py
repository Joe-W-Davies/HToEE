    ...
    with open(options.config, 'r') as config_file:
        ...
        if options.hp_perm is not None:
           ...
           if options.batch_boost: #type of model selection so need validation set
               ...
               LSTM.train_network(epochs=4, batch_size=128)
               #LSTM.train_network(epochs=7, batch_size=32)
               LSTM.save_model(out_tag=output_tag)
           AUC = LSTM.compute_roc(batch_size=1024)
           #compute final roc on test set
           LSTM.plot_roc(output_tag, AUC=AUC)
           LSTM.plot_output_score(output_tag, batch_size=1024, ratio_plot=False, norm_to_data=(not options.pt_reweight),log=True, merge_bkg_procs={'ttbarTotal':['TT2L2Nu','TTSemiL']})
